...
public class RemoteHendlerRMI implements Closeable {
    ...
    private void process(String stringPathRandFile, String stringPathSortFile) {
        ...
        Compute.HeapSort heapSort = null;
        Compute.Sum sum = null;
        try {
            heapSort = new Compute.HeapSort(fileSortNum.getName(), fileRandomNum);
        ...
        try {
            bytes = proxy.executeTask(heapSort);
            proxy.executeTask(sum);
        } catch (RemoteException e) {
        ...
        try {
            long timeProcesMethod = proxy.timeProcesMethod(heapSort);
            responseInfo = new String("Done! Time proces algoritm QuickSort = " + timeProcesMethod + " nsec| " + timeProcesMethod / 1000 + " mksec| " + timeProcesMethod / 1000000 + " msec.");
        } catch (RemoteException e) {
            responseInfo = new String("There was an server error processing!");
        }
    }
